...
class Motion(object):
    ...
    def __calculate_motor_rotation(self, distance):
        ...
        return (180.0 * distance) / (math.pi * (self.wheel_diameter / 2.0))
    ...
    def __turn_in_place(self, yaw, power = 75):
        ...
        turn_distance = (math.pi / 180.0) * abs(yaw) * (self.axle_length / 2.0)
    ...
    def follow_line_until_location(self, location):
        """
        Follow line.
    ...
    def move_until_location(self, location):
        state = self.kb.get_state()
        current_location = state['current_location']
        
        while not location == current_location:
            motion_status = self.follow_line_until_location(location)
