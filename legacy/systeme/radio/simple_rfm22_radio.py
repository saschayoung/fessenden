#!/usr/bin/env python
"""
Simple RFM22 FSK radio implementation
...
class SimpleRadio(RadioBaseClass):
    """
    Create and manage a radio instance.

    Parameters
    ----------
    RadioBaseClass : class instance
        Abstract radio base class/template.

    """

    def __init__(self, tx_port=138, rx_port=139, int_port=157, spi_dev="dev/spidev4.0"):
        """
        Radio class constructor.

        This function instantiates and initializes a Radio object.

        Parameters
        ----------
        tx_port : int, optional
            GPIO port to be used for TX switching of T/R Switch. Default is 138.
        rx_port : int, optional
            GPIO port to be used for RX switching of T/R Switch. Default is 139.
        int_port : int, optional
            GPIO port to be used for interrupt signalling. Default is 157.
        spi_dev: str, optional
            Kernel spi_dev device to use for SPI communications. Default is "dev/spidev4.0".
        
        """

        # TODO: add some way input type checking, excpetion, other whatever, etc.
    ...
    def _init_io(self):
        """
        Initiallize I/O lines.

        """
        
        # TODO: add error handling for spi_dev, or is this handled at a lower lever?
        spi.setup(self.spi_dev)

        s = "echo " + str(self.tx_port) + " > /sys/class/gpio/export"
        result = subprocess.call(s, shell=True)
        s = "echo out > /sys/class/gpio/gpio" + str(self.tx_port) + "/direction"
        result = subprocess.call(s, shell=True)
        s = "echo 0 > /sys/class/gpio/gpio" + str(self.tx_port) + "/value"
        result = subprocess.call(s, shell=True)

        s = "echo " + str(self.rx_port) + " > /sys/class/gpio/export"
        result = subprocess.call(s, shell=True)
        s = "echo out > /sys/class/gpio/gpio" + str(self.rx_port) + "/direction"
        result = subprocess.call(s, shell=True)
        s = "echo 0 > /sys/class/gpio/gpio" + str(self.rx_port) + "/value"
        result = subprocess.call(s, shell=True)

        s = "echo " + str(int_port) + " > /sys/class/gpio/export"
        result = subprocess.call(s, shell=True)
    ...
    def rx_mode(self):
        ...
        s = "echo 1 > /sys/class/gpio/gpio" + str(self.rx_port) + "/value"
        result = subprocess.call(s, shell=True)
    ...
    def close_gpio(self):
        s = "echo " + str(self.tx_port) + " > /sys/class/gpio/unexport"
        result = subprocess.call(s, shell=True)

        s = "echo " + str(self.rx_port) + " > /sys/class/gpio/unexport"
        result = subprocess.call(s, shell=True)

        s = "echo " + str(int_port) + " > /sys/class/gpio/unexport"
        result = subprocess.call(s, shell=True)
